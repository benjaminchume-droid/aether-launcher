...
class AetherEdgeOverlay(private val service:AetherOverlayService){
    ...
    fun show(){
        ...
        val lp=WindowManager.LayoutParams(dp(56),WindowManager.LayoutParams.MATCH_PARENT,type,WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE or WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS,PixelFormat.TRANSLUCENT).apply{gravity=Gravity.RIGHT or Gravity.CENTER_VERTICAL}
        params=lp
        wm.addView(frame,lp)
    }
    private fun toggle(){val r=root?:return;val lp=params?:return;if(menu!=null){r.removeView(menu);menu=null;lp.width=dp(56);wm.updateViewLayout(r,lp);return};val p=LinearLayout(service).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(12),dp(12),dp(12),dp(12));setBackgroundColor(0xEF12151A.toInt());elevation=dp(18).toFloat()};val items=listOf("Home" to {AetherSystemActions.launchHome(service)},"History" to {AetherSystemActions.openSurface(service,AetherSurface.HISTORY)},"Notifications" to {AetherSystemActions.openSurface(service,AetherSurface.NOTIFICATIONS)},"Control Center" to {AetherSystemActions.openSurface(service,AetherSurface.CONTROL)},"Widgets" to {AetherSystemActions.openSurface(service,AetherSurface.WIDGETS)},"Multitask" to {AetherSystemActions.openSurface(service,AetherSurface.MULTITASK)},"Quick Space" to {AetherSystemActions.openSurface(service,AetherSurface.QUICK)},"Settings" to {AetherSystemActions.openSettings(service)});items.forEach{(label,action)->p.addView(TextView(service).apply{text=label;textSize=14f;setTextColor(Color.WHITE);gravity=Gravity.CENTER_VERTICAL;setPadding(dp(16),0,dp(16),0);setOnClickListener{action()}},LinearLayout.LayoutParams(dp(248),dp(52)).apply{bottomMargin=dp(6)})};r.addView(p,FrameLayout.LayoutParams(dp(268),max(dp(8),r.height-dp(80)),Gravity.RIGHT or Gravity.TOP).apply{topMargin=dp(24);rightMargin=dp(58)});menu=p;lp.width=dp(300);wm.updateViewLayout(r,lp);p.translationX=dp(30).toFloat();SpringAnimation(p,DynamicAnimation.TRANSLATION_X).apply{spring=SpringForce(0f).apply{stiffness=520f;dampingRatio=.78f}}.start()}
    fun hide(){root?.let{runCatching{wm.removeView(it)}};root=null;menu=null;params=null}
}
