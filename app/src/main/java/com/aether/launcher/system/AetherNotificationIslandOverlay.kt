...
class AetherNotificationIslandOverlay(private val service: AetherOverlayService) {
 private val wm=service.getSystemService(WindowManager::class.java);private val store=AetherSettingsStore(service)
 private var view:CapsuleView?=null;private var lp:WindowManager.LayoutParams?=null;private var expanded=false;private var lastKey="";private val d get()=service.resources.displayMetrics.density;private fun dp(v:Int)=(v*d).toInt()
 fun show(){if(view!=null||!Settings.canDrawOverlays(service))return;val c=store.load().island;val v=CapsuleView(service);val type=if(Build.VERSION.SDK_INT>=26)WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY else WindowManager.LayoutParams.TYPE_PHONE;val p=WindowManager.LayoutParams(dp(c.width.coerceAtLeast(92)),dp(c.height.coerceAtLeast(28)),type,flags(false),android.graphics.PixelFormat.TRANSLUCENT).apply{gravity=Gravity.TOP or Gravity.CENTER_HORIZONTAL;softInputMode=WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE};place(p);runCatching{wm.addView(v,p);view=v;lp=p;v.setOnClickListener{toggle()};refresh()}}
 private fun flags(f:Boolean)=if(f)WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL else WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE or WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL
 ...
 fun refresh(){val v=view?:return;val a=AetherRuntime.registry.island.activity;if(a.key!=lastKey&&lastKey.isNotBlank())expanded=false;lastKey=a.key;v.activity=a;v.replyable=a.type==ActivityType.NOTIFICATION&&NotificationReplyBridge.canReply(a.key);val active=a.type!=ActivityType.NONE&&a.title.isNotBlank();if(!active)expanded=false;val c=store.load().island;pSize(if(expanded&&active)dp(min(380,max(c.width+100,270)))else dp(c.width.coerceAtLeast(92)),if(expanded&&a.type==ActivityType.NOTIFICATION)dp(176)else if(expanded&&active)dp(max(c.height+54,84))else dp(c.height.coerceAtLeast(28)))}
 private fun pSize(w:Int,h:Int){val p=lp?:return;val v=view?:return;p.width=w;p.height=h;place(p);runCatching{wm.updateViewLayout(v,p)}}
 private fun toggle(){val a=AetherRuntime.registry.island.activity;if(a.type==ActivityType.NONE)return;expanded=!expanded;lp?.let{it.flags=flags(expanded);runCatching{wm.updateViewLayout(view,it)}};refresh();if(expanded&&a.type==ActivityType.NOTIFICATION&&NotificationReplyBridge.canReply(a.key))view?.postDelayed({view?.focusReply()},100)}
...
private class CapsuleView(c:Context):FrameLayout(c){var activity=IslandActivity(ActivityType.NONE,"");var replyable=false;private val p=Paint(Paint.ANTI_ALIAS_FLAG);private var input:EditText?=null;private var send:TextView?=null;private fun dp(v:Int)=(v*resources.displayMetrics.density).toInt();init{setWillNotDraw(false)}
 ...
 override fun onSizeChanged(w:Int,h:Int,ow:Int,oh:Int){super.onSizeChanged(w,h,ow,oh);if(replyable&&h>100*resources.displayMetrics.density)composer()else clearComposer()}
 ...
 override fun onDraw(c:Canvas){val d=resources.displayMetrics.density;val w=width.toFloat();val h=height.toFloat();val r=min(h/2,30*d);p.color=0xF30A0C10.toInt();c.drawRoundRect(RectF(.5f,.5f,w-.5f,h-.5f),r,r,p);p.style=Paint.Style.STROKE;p.strokeWidth=max(1f,d);p.color=0x50FFFFFF;c.drawRoundRect(RectF(1,1,w-1,h-1),r,r,p);p.style=Paint.Style.FILL;val ex=h>55*d;val active=activity.type!=ActivityType.NONE&&activity.title.isNotBlank();p.color=Color.WHITE;p.textSize=(if(ex)12 else 10)*d;p.typeface=android.graphics.Typeface.DEFAULT_BOLD;p.textAlign=Paint.Align.LEFT;c.drawText(if(active)activity.title.take(if(ex)36 else 24)else "AETHER",38*d,min(h*.54f,23*d),p);if(!ex)return;p.typeface=android.graphics.Typeface.DEFAULT;p.textSize=9*d;p.color=0xBFFFFFFF.toInt();if(activity.detail.isNotBlank())c.drawText(activity.detail.take(48),38*d,42*d,p);p.textSize=10*d;c.drawText(if(replyable)"Quick reply • stays until notification changes"else "Tap to open activity",38*d,74*d,p)}}
