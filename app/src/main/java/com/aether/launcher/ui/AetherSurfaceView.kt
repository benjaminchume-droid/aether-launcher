...
class AetherSurfaceView(c:Context,private val surface:String,private val folderId:String?):View(c){
 ...
 private fun folder(c:Canvas,w:Float){val f=folders.load().firstOrNull{it.id==folderId};header(c,f?.name?:"Folder","Drop apps together on Home to build folders");f?.packages.orEmpty().mapNotNull{pkg->apps.firstOrNull{it.packageName==pkg}}.forEachIndexed{i,a->{val x=18f*d+(i%3)*((w-36f*d-20f*d)/3+10f*d);val y=120f*d+(i/3)*100f*d;val cell=(w-36f*d-20f*d)/3;glass(c,RectF(x,y,x+cell,y+84f*d),22f*d);icon(c,a.packageName,RectF(x+cell/2-25f*d,y+8f*d,x+cell/2+25f*d,y+58f*d));p.textSize=10f*d;p.color=Color.WHITE;c.drawText(a.label.take(13),x+cell/2,y+75f*d,p)}}}
 ...
 private fun calculator(){val input=EditText(context);input.hint="12 + 8 * 2";AlertDialog.Builder(context).setTitle("Calculator").setView(input).setPositiveButton("Calculate"){_,_->Toast.makeText(context,evaluate(input.text.toString()),Toast.LENGTH_SHORT).show()}.setNegativeButton("Close",null).show()}
