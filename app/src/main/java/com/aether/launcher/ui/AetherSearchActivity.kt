...
class AetherSearchActivity : Activity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        val query = EditText(this).apply { hint="Search apps…"; setSingleLine(); textSize=18f; setTextColor(Color.WHITE); setHintTextColor(0x75FFFFFF); setPadding(dp(18),0,dp(18),0); background=glass(0x82111820.toInt(),22f) }
        ...
        root.addView(body, android.view.ViewGroup.LayoutParams(-1,-1))
        setContentView(root)
        query.requestFocus()
        query.post { (getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager).showSoftInput(query, InputMethodManager.SHOW_IMPLICIT) }
    ...
    private fun addResult(app: AppInfo) {
        val row = LinearLayout(this).apply {
            gravity=Gravity.CENTER_VERTICAL; setPadding(dp(10),dp(6),dp(14),dp(6)); background=glass(0x6F111820.toInt(),20f)
            setOnClickListener { history.record(app.packageName); AetherRuntime.registry.launcher.launchIntent(app.packageName)?.let(::startActivity); finish() }
        }
    ...
    private fun glass(base: Int, radius: Float) = android.graphics.drawable.GradientDrawable().apply { setColor(base); cornerRadius=dp(radius.toInt()).toFloat(); setStroke(dp(1),0x45FFFFFF.toInt()) }
